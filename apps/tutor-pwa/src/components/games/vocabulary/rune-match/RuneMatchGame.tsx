...
export function RuneMatchGame({ vocabulary, tutorialMode = false, tutorialStep = 0, onComplete }: RuneMatchGameProps) {
  ...
  const lastTutorialStepRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    if (!tutorialMode || lastTutorialStepRef.current === tutorialStep) return;
    ...
    if (tutorialStep === 2) {
      ...
      const move = state ? findPossibleMoves(state.grid)[0] : undefined;
      if (!move) return;
  ...
  }, [gameState, handleCellClick, handleFreeze, handleHint, tutorialMode, tutorialStep]);
