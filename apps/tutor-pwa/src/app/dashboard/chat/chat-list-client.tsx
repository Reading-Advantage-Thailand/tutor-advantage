...
export function ChatListClient({ conversations }: { conversations: Conversation[] }) {
        ...
        {filteredConversations.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 bg-card border border-dashed border-border/60 rounded-3xl animate-scale-in">
            ...
            <p className="font-bold text-foreground">
              {searchQuery ? "ไม่พบการสนทนาที่ค้นหา" : t("dashboardChat.emptyTitle")}
            </p>
            <p className="text-sm font-medium text-muted-foreground mt-1">
              {searchQuery ? "ลองค้นหาด้วยคำอื่น" : t("dashboardChat.emptyDescription")}
            </p>
          </div>
        )}
