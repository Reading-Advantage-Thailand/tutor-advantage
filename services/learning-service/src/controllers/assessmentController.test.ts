...
import { FORMS } from "../services/origins2Assessment";

const { db } = vi.hoisted(() => ({ db: {
  classBookCycle: { findUnique: vi.fn() }, enrollment: { findFirst: vi.fn(), findMany: vi.fn() },
  assessmentWindow: { findUnique: vi.fn(), upsert: vi.fn() },
...
const answers = Object.fromEntries(FORMS.PRE.map(i => [i.id, i.correct]));
const cycle = { classBookCycleId: cycleId, classId: "class", sequence: 1, bookId: "book", book: { bookCode: "Primary Origins 2", levelNumber: 20, series: { code: "PRIMARY-ORIGINS" } }, class: { bookId: "book", tutorUserId: tutorId, isDemo: false } };
const attempt = { attemptId, studentUserId: studentId, stage: "PRE", formVersion: "primary-origins2-v1", startedAt: new Date(), submittedAt: null, scores: null, total: null, teacherComment: null, commentedAt: null };
const app = express();
...
describe("Origins 2 assessment HTTP contracts and access", () => {
  ...
  it("hides the pilot for other books and rejects starting them", async () => {
