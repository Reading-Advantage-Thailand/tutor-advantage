...
async function getLessonSessionDetails(req, res) {
    try {
        const userId = req.user?.userId;
        ...
        const participant = await database_1.prisma.sessionParticipant.findUnique({
        ...
        const articleInfo = await (0, ReadingAdvantageDB_1.getArticleDetails)(participant.session.articleId).catch(() => null);
