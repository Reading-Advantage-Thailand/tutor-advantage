...
async function createClass(req, res) {
    try {
        ...
        if (!book && !isUuid) {
            ...
            let mockSeries = await database_1.prisma.series.findFirst({
                where: { code: "MOCK-X" },
            });
            if (!mockSeries) {
                mockSeries = await database_1.prisma.series.create({
                    data: {
                        code: "MOCK-X",
            ...
            book = await database_1.prisma.book.create({
                data: {
                    bookCode: bookId,
                    title: `Book: ${bookId}`,
                    levelNumber: 1,
...
async function getClassById(req, res) {
    try {
        ...
        const role = req.user?.role;
        const isEnrolled = cls.enrollments?.some((e) => e.studentUserId === userId);
        ...
        const mapped = {
            ...
            articleId: cls.book?.bookCode
                ? (await database_1.prisma.article.findFirst({ where: { bookId: cls.bookId } }))
                    ?.articleId
                : "article-default-123",
