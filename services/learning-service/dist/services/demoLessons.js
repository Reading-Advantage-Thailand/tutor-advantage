...
exports.DEMO_LESSONS = void 0;
exports.getDemoLessons = getDemoLessons;
exports.getDemoArticle = getDemoArticle;
exports.isDemoArticleId = isDemoArticleId;
// One lesson per level. Add new levels here — the picker and socket both read this.
exports.DEMO_LESSONS = [
    {
        articleId: "demo-a1",
        level: 1,
        cefr: "A1",
        article: {
            ...
            passage: "Dolphins are smart animals that live in the ocean. They swim fast and jump high above the water. Dolphins talk to each other with clicks and whistles. Sometimes a dolphin helps a person who is lost at sea. People love to watch dolphins play near boats.",
            cefr_level: "A1",
            ra_level: "Level 1",
            genre: "Animals",
            words: [
    ...
    },
    {
        ...
        article: {
            ...
            passage: "Bees are tiny insects, but they play a huge role in our lives. When bees move from flower to flower, they carry pollen and help plants produce fruit and seeds. Without bees, many crops we eat every day would disappear. Unfortunately, the number of bees is falling because of pesticides and the loss of wild flowers. We can protect bees by planting flowers and avoiding harmful chemicals in our gardens.",
            cefr_level: "B1",
            ra_level: "Level 3",
            genre: "Science",
            words: [
...
function getDemoLessons() {
    return exports.DEMO_LESSONS.map((lesson) => ({
...
function getDemoArticle(articleId) {
    ...
    const lesson = exports.DEMO_LESSONS.find((item) => item.articleId === articleId);
    return lesson ? JSON.parse(JSON.stringify(lesson.article)) : null;
}
function isDemoArticleId(articleId) {
    return exports.DEMO_LESSONS.some((item) => item.articleId === articleId);
}
