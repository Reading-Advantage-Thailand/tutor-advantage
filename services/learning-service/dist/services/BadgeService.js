...
async function fetchTutorMetrics(tutorUserId) {
    ...
    for (const msg of chatMessages) {
        if (msg.senderId === tutorUserId) {
            ...
            if (pending) {
                responseTimes.push(Math.max(0, Math.round((msg.createdAt.getTime() - pending.getTime()) / 60000)));
    ...
    const studentSuccessRate = totalAnswers > 0 ? correctAnswers / totalAnswers : null;
    return {
        totalHours,
        avgResponseMinutes,
        ratingAvg,
 ...
 */
async function checkAndUnlockBadges(tutorUserId) {
    try {
        ...
        if (!alreadyUnlocked.has("ELITE_EDUCATOR") &&
        ...
        await prisma.$transaction(toUnlock.map((badgeCode) => prisma.tutorBadge.upsert({
 ...
 */
async function getTutorBadgeBonusSatang(tutorUserId) {
    const badges = await prisma.tutorBadge.findMany({
        where: { tutorUserId },
        select: { badgeCode: true },
    });
    return badges.reduce((sum, b) => {
        return sum + (exports.BADGE_BONUS_SATANG[b.badgeCode] ?? 0n);
    }, 0n);
}
