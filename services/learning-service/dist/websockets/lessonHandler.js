...
const setupLessonSocket = (io) => {
    io.on("connection", (socket) => {
        ...
        socket.on("join_session", ({ pin, studentId, name, pictureUrl }) => {
            const session = LessonSessionService_1.lessonSessionService.joinSession(pin, studentId, name, socket.id, pictureUrl);
            if (session) {
                socket.join(session.sessionId);
                socket.emit("join_success", {
                    sessionId: session.sessionId,
        ...
        socket.on("change_phase", ({ sessionId, phase }) => {
            const session = LessonSessionService_1.lessonSessionService.setPhase(sessionId, phase);
            if (session) {
                    ...
                    if (result.session.currentPhase === 8 || result.session.currentPhase === 13) {
                        ...
                        if (result.session.currentPhase === 7) {
                            ...
                            if (targetSentence) {
                                ...
                                const vocabWords = result.session.articleData?.words?.map((w) => w.vocabulary || w.word || w.text) || ["Apple", "Banana", "Cat"];
                                const distractors = vocabWords.filter((w) => w.toLowerCase() !== correctWord.toLowerCase());
...
exports.setupLessonSocket = setupLessonSocket;
